#!/bin/bash
echo "+++ Build images..."
docker build -t populate-consul-config -f Dockerfile.consul . 
...
docker stop facade-service logging-service1 logging-service2 logging-service3 messages-service1 messages-service2 populate-consul-config
docker rm facade-service logging-service1 logging-service2 logging-service3 messages-service1 messages-service2 populate-consul-config
echo "+++ successfully stoped and removed old services"

echo "+++ Starting populating Consul KV entries..."
export CONSUL_HOST="consul-server"
export CONSUL_PORT=8500

docker run -d \
  --name populate-consul-config \
  --network microservices-network \
  populate-consul-config \
  python3 /app/populate_consul_config.py

echo "+++ Consul KV entries populated successfully."
